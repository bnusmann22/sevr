...
type CreateProjectModalProps = { isOpen: boolean; onClose: () => void; onCreated: (name: string, description: string, defaultTlp: TLPLabel) => void };
...
export default function CreateProjectModal({ isOpen, onClose, onCreated }: CreateProjectModalProps) {
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/50 p-4" role="dialog" aria-modal="true" aria-labelledby="create-project-title">
    <form onSubmit={submit} className="w-full max-w-lg space-y-4 border border-slate-200 bg-white p-6 shadow-xl">
      ...
      <div className="flex justify-end gap-2"><button type="button" onClick={onClose} className="px-3 py-2 text-xs font-semibold text-slate-600 hover:bg-slate-100">Cancel</button><button type="submit" className="bg-slate-900 px-3 py-2 text-xs font-semibold text-white hover:bg-slate-700">Create project</button></div>
    </form>
  </div>;
}