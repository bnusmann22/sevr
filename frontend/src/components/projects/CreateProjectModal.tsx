...
export default function CreateProjectModal({ isOpen, isSubmitting = false, onClose, onCreated }: CreateProjectModalProps) {
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/50 p-4" role="dialog" aria-modal="true" aria-labelledby="create-project-title">
    <form onSubmit={submit} className="w-full max-w-lg space-y-4 border border-slate-200 bg-white p-6 shadow-xl">
      ...
      <label className="block text-xs font-semibold text-slate-700" htmlFor="create-project-name">Project name<input id="create-project-name" value={name} onChange={(event) => setName(event.target.value)} className="mt-1 w-full border border-slate-300 px-3 py-2 text-sm focus:border-emerald-600 focus:outline-none" /></label>
      <label className="block text-xs font-semibold text-slate-700" htmlFor="create-project-desc">Description<textarea id="create-project-desc" value={description} onChange={(event) => setDescription(event.target.value)} rows={3} className="mt-1 w-full border border-slate-300 px-3 py-2 text-sm focus:border-emerald-600 focus:outline-none" /></label>
